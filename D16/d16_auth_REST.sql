...
ords.create_service(
      ...
      p_pattern =>  'vendita/' ,
      p_items_per_page => 25,
      p_source  =>  'select * from d16_vendita order by id');
commit;
end;
/
/*
...
*/
begin
  ords.create_role('d16.ROLE');     
 
  ords.create_privilege(
      p_name => 'd16.PRIVILEGE',
      p_role_name => 'd16.ROLE',
      p_label => 'd16 data',
      p_description => 'Provide access to d16 data');
  commit;
end;
/

--select id,name from user_ords_privileges where name = 'd16';

begin
 ords.create_privilege_mapping(
      p_privilege_name => 'd16.PRIVILEGE',
      p_pattern => '/d16/*');     
  commit;
end;
/
...
begin 
 oauth.create_client(
      ...
      p_privilege_names => 'd16.PRIVILEGE',
      p_support_email =>'pippo@pippo.com');
 commit;
end;
/

--select name, client_id,client_secret from user_ords_clients where name = 'client_d16';
...
begin 
 oauth.grant_client_role(
     'client_d16',
     'oracle.dbtools.role.autorest.WKSP_MA0.d16_VENDITA');
 commit;
end;
/

--select * from user_ords_client_roles where client_name = 'client_d16';
https://cnde8nae4maapcd-oramms.adb.eu-zurich-1.oraclecloudapps.com/ords/ma0/d16/vendita/
...
BEGIN
    apex_web_service.oauth_authenticate_credential(
        ...
        p_credential_static_id => 'client_d16');
        l_response := APEX_WEB_SERVICE.MAKE_REST_REQUEST(p_url => 'https://cnde8nae4maapcd-oramms.adb.eu-zurich-1.oraclecloudapps.com/ords/ma0/d16/vendita/',
                                                         p_http_method => 'GET',
